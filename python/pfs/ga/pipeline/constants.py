...
class Constants():
    ...
    PFSGACONFIG_FILENAME_FORMAT = f'pfsGAConfig-{PFSOBJECT_ID_FORMAT}.yaml'
    PFSGACONFIG_FILENAME_REGEX = r'pfsGAConfig-(?P<catId>\d{5})-(?P<tract>\d{5})-(?P<patch>[0-9a-zA-Z]+)-(?P<objId>[0-9a-fA-F]{16})-(?P<nVisit>\d{3})-0x(?P<pfsVisitHash>[0-9a-fA-F]{16}).(yaml|json|py)'
